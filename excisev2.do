...
// NEW: PRICE FROM BPS
// pk = price of cigarette clove
// pkf = price of cigarette clove + filter
// pp = price of normal cig no clove
gen lpk=log(pk)
gen lpkf=log(pkf)
gen lpp=log(pp)

// NEW: AVE2 where AVE use this price


gen apk=tskt/pk
gen apkf=tskm/pkf
gen appp=tspm/pp
gen lapk=log(tskt/pk)
gen lapkf=log(tskm/pkf)
gen lappp=log(tspm/pp)
...
// NEW: graphing the HTP and BPS Price (note that HTP its has plenty missing spaces)
...
global bps lpkf lpp lpk lgni
global abps apkf appp apk lgni
global labps lapkf lappp lapk lgni
...
sureg (lqskm $bps) (lqspm $bps) (lqskt $bps)
outreg2 using "reg/qbps", word excel replace

sureg (lqskm $abps) (lqspm $abps) (lqskt $abps)
outreg2 using "reg/abps", word excel replace

sureg (lqskm $labps) (lqspm $labps) (lqskt $labps)
outreg2 using "reg/labps", word excel replace

sureg (lpkf $labps) (lpp $labps) (lpk $labps)
outreg2 using "reg/passthrough", word excel replace
